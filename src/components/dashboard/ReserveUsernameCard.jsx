...
export default function ReserveUsernameCard({ userEmail }) {
  ...
  const { data: reserved = [] } = useQuery({
    queryKey: ["reserved", userEmail],
    queryFn: async () => {
      const { data } = await supabase.from("reserved_usernames").select("*").eq("user_email", userEmail).order("created_at", { ascending: false });
      return data || [];
    }, enabled: !!userEmail
  });

  const { data: todayCount = 0 } = useQuery({
    queryKey: ["today-gen", userEmail],
    queryFn: async () => {
      const today = new Date(); today.setHours(0,0,0,0);
      const { count } = await supabase.from("reserved_usernames").select("*", { count: "exact", head: true }).eq("user_email", userEmail).gte("created_at", today.toISOString());
      return count || 0;
    }, enabled: !!userEmail
  });
  ...
  const generate = async () => {
    ...
    const nu = []; let a = 0;
    while (nu.length < effectiveCount && a < 100) {
      const g = gen();
      if (!ex.has(g)) { nu.push({ user_email: userEmail, gmail_address: g }); ex.add(g); }
      a++;
    }
    if (nu.length > 0) {
      await supabase.from("reserved_usernames").insert(nu);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-card border border-border rounded-xl p-5 md:p-6">
      <div className="silver-gradient rounded-lg py-2 px-4 mb-4 text-center">
        <h3 className="font-orbitron text-sm font-bold text-black uppercase">Reserve Usernames</h3>
      </div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <Button onClick={generate} disabled={busy || atLimit} className="bg-primary text-primary-foreground">
          {busy ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <UserPlus className="w-4 h-4 mr-2" />}
          {atLimit ? "Daily Limit Reached" : "Generate"}
        </Button>
        {reserved.length > 0 && (
          <Button onClick={() => { navigator.clipboard.writeText(reserved.map(r => r.gmail_address).join("\n")); toast.success("Copied!"); }} variant="outline" size="sm">
            <Copy className="w-4 h-4 mr-1" />Copy
          </Button>
        )}
      </div>
      <p className="text-xs text-muted-foreground mb-3">Today: {todayCount}/{DAILY_GEN_LIMIT} generated</p>
      {reserved.length > 0 && (
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {reserved.map(r => (
            <div key={r.id} className="flex items-center justify-between bg-secondary/50 rounded-lg px-3 py-2">
              <span className="text-xs font-mono">{r.gmail_address}</span>
              <button onClick={() => { navigator.clipboard.writeText(r.gmail_address); toast.success("Copied!"); }} className="text-muted-foreground hover:text-primary"><Copy className="w-3 h-3" /></button>
            </div>
          ))}
        </div>
      )}
